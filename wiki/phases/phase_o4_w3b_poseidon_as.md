...
| Existing Poseidon-using helpers | `bridge/zk_artifacts/compute_inputs.js` + `compute_inputs_pitl.js` + `compute_inputs_passport.js` | Reference invocation pattern (`buildPoseidon()` + `F.toObject()`) |
| ZK-SEPPROOF circuit | `contracts/circuits/ZKSepProof.circom:166` (`Poseidon(5)`) | Arity reference for ZK-SEPPROOF binding |
| PitlSessionProof circuit | `contracts/circuits/PitlSessionProof.circom:106` (`Poseidon(8)`) + `:139` (`Poseidon(2)`) | Arity references for featureCommitment + nullifierHash |
...
```
   ...
   I.2 (byte interface helper) → 32-byte ↔ field conversion
         ↓
   V.1 (vector match) → +10 tests; all arities × full vector set; AS output == reference
   V.2 (per-round differential) → +6 tests; 50 vectors/arity; intermediate states match
   V.3 (cross-reference) → +2 tests; AS == ref1 AND AS == ref2 byte-identical
         ↓
   PV.1 (governance ceremony 83 → 86) → operator three-factor for 3 new INVs
         ↓
   W.1 (wire validate_poac_record.ts) → replace placeholder fills
   W.2 (wire validate_zk_sepproof.ts) → secondary target
   W.3 (audit script update) → move POSEIDON_HASH to CLOSED_DELTAS
         ↓
   T.1 (test alignment) → update 4 + add 6 tests (cumulative +24 across V.* + T.1)
         ↓
...
**Goal:** Produce canonical test vector file from circomlibjs reference for all 3 arities (t=3 / t=6 / t=9 — circomlib internal-state sizes for `Poseidon(2)` / `Poseidon(5)` / `Poseidon(8)` respectively).

| Commit | Scope | Risk |
|---|---|---|
| P.2 | New `scripts/w3bstream/poseidon_vector_generator.js` (Node.js script using `buildPoseidon()` from vendored circomlibjs). Generates: (a) 1000 random-input vectors per arity {t=3, t=6, t=9}; (b) ~30 boundary-input vectors per arity (zero, p-1, all-ones, sequential ramp, hand-picked collision-test inputs from circomlib's own test corpus); (c) 50 vectors per arity with per-round intermediate state captures (for V.2). Output: `scripts/w3bstream/poseidon_test_vectors.json` (~5 MB; vector arrays + per-round states + metadata header documenting circomlibjs version + BN254 prime literal + R_F/R_P per arity). SHA-256 of vector file computed and printed; file's SHA-256 will be pinned by INV-POSEIDON-AS-003. **PARAMETER-SET VERIFICATION (Risk #1 boundary gate):** P.2 additionally produces ONE canonical vector matching what a deployed circuit (Phase 237 ZK-SEPPROOF or Phase 62 PitlSessionProof) would emit at runtime for a specific documented test input, then verifies that canonical vector against the deployed verifier's expected output via Hardhat call or direct contract read (`Groth16VerifierZKSepProof` at `0xD63EEf1372Cb...` is the ground-truth reference). If parameter-set verification FAILS, plan halts at P.2 with documented diagnosis: which circuit emitted incompatible output, which parameter components drifted (round constants / MDS matrix / R_F/R_P), and what alternative reference would match. P.2 commit body documents the parameter-set verification outcome + the specific test input used + the verifier contract address consulted. This catches Risk #1 at the P.2 boundary BEFORE substantial I.* work is sunk cost. | LOW (read-only references; produces single data file; adds one read-only contract verification call) |
...
**Pre-commit verification:** Re-run generator from clean state; vector file byte-identical to checked-in version (determinism check). Parameter-set verification step PASSES against deployed verifier `Groth16VerifierZKSepProof` (Risk #1 boundary gate cleared). PV-CI PASS.
...
**Goal:** Implement Poseidon(BN254) in AssemblyScript for arities t∈{3,6,9} (covering `Poseidon(2)/Poseidon(5)/Poseidon(8)`).
...
| I.1b | Add Poseidon permutation (full + partial rounds); round constants table emitted as `const` byte arrays generated by P.2's vector script (parallel output: a separate `poseidon_constants_gen.js` writes `scripts/w3bstream/poseidon_constants.generated.ts` from circomlibjs Grain LFSR output); MDS matrix per arity; three hash entry points: `poseidon_t3(in0: ArrayBuffer, in1: ArrayBuffer): ArrayBuffer`, `poseidon_t6(inputs: ArrayBuffer): ArrayBuffer` (5×32B inputs), `poseidon_t9(inputs: ArrayBuffer): ArrayBuffer` (8×32B inputs). All return canonical 32-byte big-endian field-element output matching circomlibjs `F.toObject()` byte order. NO applet wiring yet. | MEDIUM-HIGH; the substantive cryptographic work |
...
| V.3 | (GREEN only; SKIP if AMBER) `T-W3B-POSEIDON-AS-17..18`: cross-reference triangulation. AS output == circomlibjs output AND AS output == 2nd-reference output for every vector in file. Disagreements between references = halt + investigate (likely a misunderstanding of arity / parameter set). | LOW given GREEN preflight pass |
...
| Commit | Scope | Risk + Safety |
|---|---|---|
| W.1 | Edit `scripts/w3bstream/validate_poac_record.ts`: import `poseidon_t9` + `poseidon_t3` from `poseidon_bn254.ts`. Replace `memory.fill(headStart + 64, 0, 32)` (featureCommitment) at line 431 with: extract scaledFeatures from PoAC body + extract inferenceCodeFromBody + call `poseidon_t9` + write 32B output. Replace `memory.fill(headStart + 160, 0, 32)` (nullifierHash) at line 441 with: deviceIdHash + epoch → `poseidon_t3` + write 32B output. **NOTE: `epoch` is still zero per separate W3bstream-runtime delta; nullifierHash becomes Poseidon(deviceIdHash, 0) deterministic per device.** Comment block at lines 392-406 updated to reflect new state (POSEIDON_HASH closed; nullifierHash anti-replay still broken pending epoch delta closure). asc compile clean. Binary size compared to P.1 baseline; document delta in commit body. | MEDIUM (real cryptographic computation now in critical path; binary size +∼KB acceptable) |
| W.2 | Mirror into `scripts/w3bstream/validate_zk_sepproof.ts` if/where POSEIDON_HASH stubs exist there. Use `poseidon_t6` for Phase 237 ZK-SEPPROOF (arity t=6 per `ZKSepProof.circom:166`). If validate_zk_sepproof.ts has no POSEIDON_HASH stubs (per first explore agent's report; verify in W.2), W.2 collapses to a no-op confirmation commit body. | LOW (secondary target; conditional) |
...
- Bridge total passing ≥ 3728 + 24 = 3752 (V.1=10 + V.2=6 + V.3=2 + T.1=6 all landed by this point in the stream order)
- PV-CI 86/86 PASS
...
1. `python -m pytest bridge/tests/ --ignore=bridge/tests/test_e2e_simulation.py -q` — passing count ≥ 3728 + 24 (V.1=10 + V.2=6 + V.3=2 + T.1=6) = ≥ 3752 projected post-T.1
...
|---|---|---|---|
| Bridge tests | 3728 passing | ≥ 3752 passing | +24 (V.1=10 + V.2=6 + V.3=2 + T.1=6) |
