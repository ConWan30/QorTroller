...
export function normalizeStreamModel(raw) {
  ...
  return {
    ...
    on_screen: {
      ...
      // STREAM-2 face fields — pass-through; missing stays null/UNKNOWN (never invent)
      node_identity: on.node_identity && typeof on.node_identity === 'object' ? on.node_identity : null,
      contribution: on.contribution && typeof on.contribution === 'object' ? on.contribution : null,
      scorecard: on.scorecard && typeof on.scorecard === 'object' ? on.scorecard : null,
      witness_blink: on.witness_blink && typeof on.witness_blink === 'object' ? on.witness_blink : null,
      node_id_short: on.node_id_short || on.node_identity?.node_id_short || null,
      kills_seen: typeof on.kills_seen === 'number' ? on.kills_seen
        : (typeof on.witness_blink?.kills_seen === 'number' ? on.witness_blink.kills_seen : null),
    },
    deliberately_absent: Array.isArray(raw.deliberately_absent) ? raw.deliberately_absent : [],
    novelty: raw.novelty || 'node_face_witness_respiration',
    mock: false,
