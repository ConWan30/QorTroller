...
Following the exact FROZEN-v1 PATTERN-017 hash-chain convention already used
by WEC (watchdog) and GIC (grind integrity) — same shape, new domain tag:
...
def genesis_manifest_hash(session_id: str, device_id_hex: str, ts_ns: int) -> bytes:
    """SHA-256(DOMAIN_TAG_GENESIS || session_id_hash(32) || device_id(32) || ts_ns_be(8))."""

def compute_manifest_entry(prev_hash: bytes, frame_hash: bytes, frame_index: int, ts_ns: int) -> bytes:
    """SHA-256(prev_hash(32) || frame_hash(32) || frame_index_be(4) || ts_ns_be(8))."""
```

- `frame_hash` = SHA-256 of the archived frame's raw pixel bytes at the
  moment it's written to disk (pristine, pre-any-recompression). This is
  the F-RWM-2-anticipated "exact-pixel hash dies under re-compression"
...
def encode_mark_symbols(payload: bytes, *, palette: list[tuple[int,int,int]] = DEFAULT_PALETTE) -> list[tuple[int,int,int]]:
    """Prepends a fixed 2-symbol sync preamble (colors reserved, never used
    in payload encoding, e.g. pure-white then pure-black) so a decoder
    scanning an arbitrary frame sequence can locate mark boundaries even
    with dropped/reordered frames -- addresses F-RWM-3's missing sync.
    Each payload symbol repeated 3x (majority-vote decode) as the baseline
    error-correction proposal -- simplest viable scheme, not full
    Reed-Solomon, because the payload is now ~96 bits (not the original
...
**Bit budget, concretely sized (resolves open questions #1-2):** 96-bit
payload (8-byte session locator + 3-byte checkpoint index + 1-byte CRC) ×3
repetition = 288 symbol-slots. At the lightbar precedent's "5-15 symbols/sec"
density, that's ~19-58 seconds per full mark cycle — slower than originally
implied by the old 25-75-bit sketch, but no longer competing with a 512-bit
signature's bit budget (Path B's number, not applicable here). **Proposed
starting point for review, not finalized:** 8 symbols/sec, ~36s per cycle,
refreshed at the manifest's own checkpoint cadence once that's confirmed
(open question #3) so the locator and the manifest never drift out of sync
with each other.
...
2. Locator: `compute_locator_payload` determinism; `encode_mark_symbols` /
   ...
   `None`, never a wrong-but-plausible payload).
