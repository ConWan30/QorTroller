...
Status: **CANDIDATE / DESIGN-ONLY — CONVERGED, grok r08 verdict PASS (residual-accepted).** No code,
no flag flips (L6B/poep_enabled stay false), no hardware change, no chain write, no FROZEN-v1
addition. Round: **r08 converged** (ASM-Loop, v2.2). Builder: Claude; adversarial auditor: grok
(8 rounds). Accepted residuals: **R-F19** (pure-passive HID is inherently replayable — optical
mandatory for the replay-resistant `CONTINUOUS_PRESENT`; operator-accepted design ceiling), **R-HYP**
(W_min/ε/thresholds are hypotheses pending U2/U3 measurement). r03/r05 baselines preserved in the
round files for audit trail. Not committed — single-committer=operator.
...
optical_optional_for_PARTIAL       = true
optical_MANDATORY_for_CONTINUOUS   = true   # R-C9: not 'optional Phase-2' — load-bearing for the strong verdict
```
...
## 7. Thesis C (optical co-presence) — OPTIONAL for PARTIAL-tier, MANDATORY for CONTINUOUS (R-C9)

**Supersession (grok r08 R-C9):** the earlier "optional Phase-2" framing is superseded. Optical is
optional only in that pure-passive play still yields an honest `PARTIAL_PRESENT` (replayable,
advisory); it is **mandatory** for the replay-resistant `CONTINUOUS_PRESENT` verdict (§2.5 F19). It is
...
- **Q3 Thesis C:** optical is optional for PARTIAL-tier operation but MANDATORY for the replay-
  resistant CONTINUOUS verdict (R-C9 supersession; §7 retitled).
...
- **C9 (revised r08, R-C9).** Thesis C (optical co-presence) is optional only for PARTIAL-tier
  operation and **MANDATORY for the replay-resistant `CONTINUOUS_PRESENT` verdict**; it uses non-HID
  (optical/killfeed) event clocks only, carries no
