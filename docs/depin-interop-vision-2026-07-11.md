...
| ioID registry | **LIVE** (contract) + **code-complete device path** | permit registry `ioIDRegistry` `0x0A7e595C…` (`nonces()` + 8-param EIP-712 `register`; F-T3-1 — NOT the Phase 55 `VAPIioIDRegistry` `0xF7885B58…` DID book); `bridge/vapi_bridge/controller_ioid_registration.py` — gamer-signed EIP-712 permit → `did:io:{device_id}` DID doc + TBA via `ioID.wallet(tokenId)`; DEVICE_ID_CANON_v1 = keccak256(65B P-256 pubkey) |
