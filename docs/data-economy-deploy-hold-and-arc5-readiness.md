...
| 4 | VAPIConsentManifestRegistry (structured 7-dim consent) | **BUILT** | **DEFERRED → SUPERSEDED: LIVE (testnet)** | `6bf75830`; `VAPIConsentManifestRegistry.sol` present; bridge read path fail-open. **SUPERSEDED 2026-07-10:** this contract was subsequently deployed — `eth_getCode(0x5F7c8068D0e61818FCD613D47e68a9Ea906a2743)` on IoTeX 4690 returns **3869 bytes** (LIVE testnet; control `VAPIConsentRegistry` = 2247 bytes confirms the RPC). The "DEFERRED" status is stale **for Arc 4's ManifestRegistry only** — Arc 2 verifier + Arc 5 verifier deploy-hold status were not re-checked here. See `docs/depin-consent-legitimacy-lane-2026-07-10.md` §4.3 / D-DEPIN-3. |
