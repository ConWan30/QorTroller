...
- **Ceremony-gate:** Arc 5 has two sequential gates before live operation: (a) trusted setup contribution producing the `.zkey` + auto-generated `Groth16VerifierVAPIReplayProof.sol` + populating `bridge/vapi_bridge/replay_proof_pipeline/zk_artifacts/`; (b) a circomlibjs node helper producing the public Poseidon commitments (vhpCommitment, sanitizedTraceRoot) byte-identically to the in-wasm Poseidon. Until both land, `DeferredProver` returns `vhr_proof_deferred` honestly — no fake proofs.
