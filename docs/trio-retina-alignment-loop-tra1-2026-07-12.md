...
|----|-------|--------------|------|
| **T0** | **Ground the standard vs. our reimplementation** | field-by-field diff of `retina.event/0.1` + `WorldState` against `retina_events_root`/`retina_state_commitment`; produce the adoption map + the FROZEN-candidate reconciliation question; kill-check adoption touches neither the 228B wire nor the ASSERTION plane | **DESK** |
| **T1** | **`retina.event/0.1` emitter/adapter** | pure adapter emitting QorTroller observation events in the **real** spec format (3 required + omit-empty optionals); validate against the spec (`retina validate`-equivalent). Runs on M17/synthetic — no card | **DESK** |
| **T2** | **`WorldState` + `locus`/`vec` fusion channel** | map the controller IMU/trigger sensor stream into `WorldState.locus` (field position) + model-tagged `vec` latent, **beside** the video entities — the multi-modal fusion expressed in MachineFi's own schema (audio stays dropped by principle) | **DESK design; live fusion card-gated** |
| **T3** | **The VERIFY rung (QorTroller's contribution)** | compute the Poseidon `events_root`/`state_commitment` **over the canonical standard WorldState bytes**; adversarial: a tampered WorldState → commitment mismatch CAUGHT. FROZEN promotion of `VAPI-RETINA-STATE-v1` (if adopted) is **operator-sealed** | **DESK build · SEAL operator** |
| **T4** | **Separation-law conformance check** | machine-checked: no emitted `retina.event`/WorldState entity carries an asserting/humanity field (encoder-only, both standards' shared discipline) — the "suggests never asserts" rail at the wire layer | **DESK** |
| **T5** | **Adoption decision — import vs. spec-only** | operator decision: `pip install trio-retina` and wrap it as the encoder (ride DINOv2/V-JEPA embedders + `retina validate` for free) **vs.** keep our own encoder emitting the spec. Design doc + tradeoffs (dep weight, Apache-2.0 compat, edge/CPU fit) | **OPERATOR DECISION** |
