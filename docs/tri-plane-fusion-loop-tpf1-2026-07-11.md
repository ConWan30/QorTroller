...
| **F3** | **Meaning-plane HARD cryptographic join** | **F1 found a cleaner path than re-hashing the bundle:** the WMP bundle already exposes `poacChainRoot` (its FROZEN Groth16 public input); surface a matching `poac_chain_root` on the PoSP side (its KAS already commits the same M17 PoAC records) → CRYPTOGRAPHIC join **without touching the published bundle**. The re-hash-the-bundle (schema-v2) variant stays the fallback. Both are **OPERATOR DECISIONS** | **HOLD for operator GO** |
