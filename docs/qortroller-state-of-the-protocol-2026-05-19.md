...
- **QorTroller namespace (FC-(a), frozen 2026-05-24):** SEPARATE from the 12 VAPI Layer-C families above — `qortroller_commitment_family_count` = **1** (③ QORTROLLER-IPACT-RENEWAL-v1, the iPACT renewal cadence family). Composite-sig v1.1 (①, a signature primitive) + QORTROLLER-IPACT-CHALLENGE-v1 (a capability) are also frozen but NOT counted as families. Audited by a parallel `mythos_qortroller_crypto_drift` scanner; the VAPI count stays 12.
