...
Honest absence is success. Never invent out_edge from HDMI or hid_edge.
Never hash a waveform.
"""
...
def canonicalize_out_edge(raw: Any) -> dict[str, Any] | None:
    ...
    if isinstance(digest, str) and _HASH.match(digest.strip()):
        ...
        if raw.get("present") in (False, None, "", 0):
            return None
        return None
    lag = raw.get("lag_ns")
    try:
...
def out_edge_from_event(ev: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    for key in ("out_edge", "pad_out", "output"):
        ...
        if block in (None, "", {}, []):
            continue
        if isinstance(block, dict) and any(k in block for k in ("crop", "frame", "pixels", "hdmi", "score")):
            continue
...
def ledger_checks(envelope: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    add("out_edge_optional", True, f"present={present} hid_on_console={hid_on_console}")
    add("ps5_empty_io_success", True, "no pad reports on this host" if hid_on_console and present == 0 else "n/a")
    return checks
