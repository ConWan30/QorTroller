...
**Proven, not promised (2026-07-11):** the first real match was packaged as a certified-human data bundle, and **anyone can verify it with one command, trusting us for nothing** — real human (zero-knowledge proof), consent granted on-chain by the gamer's own signature, and the data cryptographically matching the proof. The bundle is published in this repo; run `python scripts/wmp_full_verify.py --bundle wmp_corpus_real/wmp_corpus.jsonl --allow-deferred recency`.
