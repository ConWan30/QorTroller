...
honest IDENTITY_ONLY; the CLI NEVER synthesizes `real_hardware=True`, activity, or PCC.
    ...
    POEP_LIVE_FIRE_ENABLED=1 python scripts/poep_session_identity_attach.py --live \\
        --wait-active-s 45 --amplitude 80 --fire-timeout 25 --challenges 2   # operator rig

Live preflight (--wait-active-s, default 45; 0=legacy cold) waits for window_n>=3 + frac>0 + PCC
ok before challenges; timeout exits 3 (no silent n_go=0). --amplitude defaults 60, clamp 1..80.
...
# Exit when --live preflight never sees ACTIVE_GAMEPLAY (honest; avoids silent n_go=0).
EXIT_PREFLIGHT_TIMEOUT = 3
_CAPTURE_HEALTH_PATH = "/operator/bridge/capture-health"
...
def _capture_health_url(bridge_url: str) -> str:
    # Measured live 2026-07-18: capture-health lives on the operator sub-app (mounted at /operator).
    return bridge_url.rstrip("/") + _CAPTURE_HEALTH_PATH


def _fetch_capture_health_raw(bridge_url: str, api_key: str, *, timeout_s: float = 3.0) -> dict:
    """GET full capture-health JSON. Fail-closed: unreachable/malformed -> {} (never invents fields)."""
    import urllib.request

    url = _capture_health_url(bridge_url)
    try:
...
def eval_preflight_ready(health: dict) -> tuple[bool, dict]:
    """SYNC-GO preflight: same fields as production activity fetcher + sealed pcc_allows_challenge.

    Ready iff ALL of:
      - capture_state / host_state pass pcc_allows_challenge (NOMINAL + EXCLUSIVE_USB|UNKNOWN)
      - live_activity_window_n >= 3
      - live_trigger_active_fraction > 0  (-> ACTIVE_GAMEPLAY under sealed classify_activity)

    Never invents fraction/PCC; empty/error health -> not ready.
    """
    if not isinstance(health, dict):
        health = {}
    n = health.get("live_activity_window_n")
    v = health.get("live_trigger_active_fraction")
    capture = health.get("capture_state")
    host = health.get("host_state")
    pcc_ok = pcc_allows_challenge({"capture_state": capture, "host_state": host})
    window_ok = isinstance(n, int) and n >= 3
    frac_val: Optional[float]
    if isinstance(v, (int, float)):
        frac_val = float(v)
    else:
        frac_val = None
    frac_ok = frac_val is not None and frac_val > 0.0
    ready = bool(pcc_ok and window_ok and frac_ok)
    status = {
        "ready": ready,
        "frac": frac_val,
        "window_n": n if isinstance(n, int) else None,
        "capture": capture,
        "host": host,
        "pcc_ok": pcc_ok,
        "window_ok": window_ok,
...
def clamp_cli_amplitude(requested: int) -> int:
    """CLI band: default 60, clamp to [1, LOW_AMPLITUDE_FORCE_MAX=80]."""
    if requested <= 0:
        return LOW_AMPLITUDE_FORCE_DEFAULT
    return max(1, min(int(requested), LOW_AMPLITUDE_FORCE_MAX))


def wait_for_active_gameplay(
    *,
    health_fetcher,
    wait_s: float,
    poll_s: float = 1.0,
    ...
    """Poll health until ACTIVE_GAMEPLAY-ready or timeout. Live-only; never fabricates readiness.

    Returns True if ready, False on timeout. Prints waiting lines to stderr.
    """
    if wait_s <= 0:
        return True
    ...
    deadline = now_fn() + float(wait_s)
    while True:
        left = deadline - now_fn()
        if left <= 0:
            break
        health = health_fetcher()
        ready, st = eval_preflight_ready(health if isinstance(health, dict) else {})
        if ready:
            print(
                f"PREFLIGHT READY: frac={st['frac']} window_n={st['window_n']} "
                f"capture={st['capture']} host={st['host']}",
                file=err,
            )
            return True
        print(
            f"waiting active play: frac={st['frac']} window_n={st['window_n']} "
            f"capture={st['capture']} host={st['host']} ({left:.0f}s left)",
            file=err,
        )
        sleep_fn(min(float(poll_s), max(0.05, left)))
    print(
        f"PREFLIGHT TIMEOUT: no ACTIVE_GAMEPLAY within {wait_s:g}s — "
        f"refusing attach (exit {EXIT_PREFLIGHT_TIMEOUT}); no GOs issued",
        file=err,
    )
    return False
...
def _make_bridge_health_fetcher(bridge_url: str, api_key: str, kind: str):
    ...
    def _fetch():
        data = _fetch_capture_health_raw(bridge_url, api_key)
        if not data:
...
def main() -> int:
    ...
    ap.add_argument(
        "--wait-active-s", type=float, default=45.0, dest="wait_active_s",
        help="live-only: wait up to N seconds for ACTIVE_GAMEPLAY (window_n>=3, frac>0, PCC ok) "
             "before challenges; 0=no wait (legacy cold attach). Timeout exits 3 (no silent n_go=0).",
    )
    ap.add_argument(
        "--poll-s", type=float, default=1.0, dest="poll_s",
        help="live preflight poll interval seconds (default 1.0)",
    )
    ap.add_argument(
        "--amplitude", type=int, default=LOW_AMPLITUDE_FORCE_DEFAULT,
        help=f"GO fire amplitude (default {LOW_AMPLITUDE_FORCE_DEFAULT}, clamp 1..{LOW_AMPLITUDE_FORCE_MAX})",
    )
    ...
    amplitude = clamp_cli_amplitude(args.amplitude)
    ...
    if live:
        ...
        if not real_hid_fire_available():
            ...
            return 2
        # SYNC-GO B1: wait for real ACTIVE_GAMEPLAY before issuing challenges (exit 3 on timeout).
        if args.wait_active_s > 0:
            ok = wait_for_active_gameplay(
                health_fetcher=lambda: _fetch_capture_health_raw(args.bridge_url, args.api_key),
                wait_s=args.wait_active_s,
                poll_s=max(0.05, float(args.poll_s)),
            )
            if not ok:
                return EXIT_PREFLIGHT_TIMEOUT
        ...
        print("[live] served by the bridge's single-HID ring: fire=/operator/poep/fire, "
              f"activity/PCC=/bridge/capture-health (same reader); amplitude={amplitude}. "
              "Bridge gates refuse fail-closed -> honest IDENTITY_ONLY when closed.")
    else:
        fire_fn = _dry_fire
        imu_fn = _dry_imu
