...
INVARIANTS: list[Invariant] = [
    ...
    Invariant(
        id="INV-RETINA-LOOP-001",
        description="Dual-Lobe framework: Thread C isolation guard enforces L0 poll rate >= 990 Hz; Screen Lobe auto-kills if poll rate degrades to preserve anti-cheat moat (F-HW-1 throughput bottleneck resolution).",
        file="bridge/vapi_bridge/retina_onnx_bridge.py",
        pattern=r'min_poll_rate_hz.*990',
        min_matches=1,
    ),
    Invariant(
        id="INV-RETINA-DATA-FLOOR",
        description="Dual-Lobe framework: FORBIDDEN_COLUMNS invariant observed - no raw pixels or biometric features enter retina_event_log (privacy floor maintained).",
        file="bridge/vapi_bridge/retina_screen_lobe.py",
        pattern=r'Fail-open|fail-open',
        min_matches=1,
    ),
    Invariant(
        id="INV-RETINA-MEDIAPIPE-DEFAULT",
        description="Dual-Lobe framework: MediaPipe integration is optional and fails-open gracefully when unavailable (no hard dependency on mediapipe package).",
        file="bridge/vapi_bridge/retina_screen_lobe.py",
        pattern=r'MEDIAPIPE_AVAILABLE.*False|except ImportError',
        min_matches=1,
    ),
    Invariant(
        id="INV-RETINA-ONNX-DEFAULT",
        description="Dual-Lobe framework: ONNX Runtime integration is optional and fails-open gracefully when unavailable (no hard dependency on onnxruntime package).",
        file="bridge/vapi_bridge/retina_onnx_bridge.py",
        pattern=r'ONNX_AVAILABLE.*False|except ImportError',
        min_matches=1,
    ),
    Invariant(
