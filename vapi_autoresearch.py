...
# Phase 238 (Decision A): bridge SQLite for FSCA findings.
# Same default as unified_server.py:81 — fail-open if DB missing.
BRIDGE_DB_PATH      = Path(os.environ.get("VAPI_DB_PATH", "bridge/vapi_store.db"))
...
def load_fsca_findings(
    severity_min: str = "HIGH",
    age_hours: int = 24,
    limit: int = 10,
) -> list:
    """Phase 238 Decision A — load active FSCA contradictions for cycle context.

    Reads `fleet_coherence_log` directly via sqlite3 (cycle script can't import
    unified_server.db_query without circular deps). Schema confirmed at
    bridge/vapi_bridge/store.py:2281–2301.

    Filtering rationale:
      - severity ≥ HIGH: A.D1 mitigation per design review §A.D1. CRITICAL+HIGH only;
        MEDIUM/LOW excluded as transient noise.
      - age ≤ 24h: stale orphans (>24h) get filtered out; matches the existing
        FSCA orphan threshold pattern.
      - resolved=0: only currently-active findings.
      - LIMIT 10: bounded prompt size; if more contradictions exist, the
        operator should investigate the cluster, not the cycle.

    Returns: list of dicts with keys (rule_name, severity, agents_involved,
    explanation, created_at). Empty list on missing DB / missing table /
    SQLite error — fail-open is required so the cycle never breaks on FSCA
    infrastructure absence.
    """
    if not BRIDGE_DB_PATH.exists():
        return []

    severity_set = {"CRITICAL", "HIGH"} if severity_min == "HIGH" else {"CRITICAL"}
    severity_placeholders = ",".join("?" * len(severity_set))

    try:
        conn = sqlite3.connect(str(BRIDGE_DB_PATH), timeout=2.0)
        conn.row_factory = sqlite3.Row
        # created_at is ISO TEXT (not Unix); use SQLite datetime() for the window.
        # store.py:2300 — created_at TEXT DEFAULT (datetime('now'))
        rows = conn.execute(
            f"""
            SELECT rule_name, severity, agents_involved, explanation, created_at
            ...
            LIMIT ?
            """,
            (*severity_set, f"-{age_hours} hours", limit),
        ).fetchall()
        conn.close()
        return [dict(r) for r in rows]
    except sqlite3.OperationalError:
        # Table missing or DB locked — fail-open
        return []
    except Exception:
        # Any other read failure — fail-open
        return []
...
def format_cycle_prompt(
    ...
    priority: str,
    cycle_num: int,
    fsca_findings: Optional[list] = None,
) -> str:
    """
    ...
    # Phase 238 Decision A — FSCA contradiction-aware prompt section.
    # Empty list renders "(none active)" so the prompt structure stays
    # uniform across cycles (Claude's parsing of the prompt template
    # doesn't have to handle a missing section).
    findings = fsca_findings or []
    if findings:
        fsca_summary = "\n".join(
            f"  [{f['severity']}] {f['rule_name']} (agents: {f.get('agents_involved','?')[:80]})\n"
            f"    Explanation: {f.get('explanation','')[:160]}"
            for f in findings
        )
    else:
        fsca_summary = "  (none active in last 24h at severity ≥ HIGH)"
...
ACTIVE FSCA CONTRADICTIONS (severity ≥ HIGH, last 24h, fleet_coherence_log):
{fsca_summary}
...
def run_cycle(
    ...
    # Phase 238 Decision A — load FSCA findings before priority selection so
    # operator visibility into contradictions matches the cycle's own context.
    fsca_findings = load_fsca_findings()
    ...
    print(f"  Active FSCA findings (severity ≥ HIGH, last 24h): {len(fsca_findings)}")

    # Generate the improvement prompt
    prompt = format_cycle_prompt(
        current_skill, program, log, priority, cycle_num, fsca_findings
    )
