...
Crops: **600** from `C:\Users\Contr\vapi-pebble-prototype\retina_kf_archive/seg3_20260701_052921`  ·  2161.1 ms/crop
...
- Instrument A OWN_KILL reads: **5**
- **Candidate false reads** (A=OWN_KILL contradicted by B seeing the handle in a non-killer slot): **1** — bar is ZERO after adjudication; see contact sheet.

## Per-instrument taxonomy
- A `ocr_row_v1` (killer-slot READER): {'UNRESOLVED': 595, 'OWN_KILL': 5}
- B `template_ensemble_v1` (template SCORER): {'OWN_KILL': 185, 'UNRESOLVED': 126, 'OTHER_ROW': 184, 'OWN_DEATH': 105}
- UNRESOLVED rate — A **99.2%** / B **21.0%** (pre-registered DRIFT ALARM: a rendering change spikes this before recall silently collapses)

## Disagreement report (the independence measurement)
{'B_KILL_A_MISS': 181, 'CONFLICT_A_KILL_B_ROSTER': 1}
...
- Crops neither instrument resolved (UNRESOLVED by BOTH): **126** — the deep tail where feed_v1 < 0.40 (A cannot locate) AND template < 0.66 (B sub-floor). This is the human_oracle contact-sheet input.
