...
class RetinaMixin:
    ...
    def insert_retina_da_upload_log(
        self,
        *,
        device_id: str,
        record_hash_hex: str,
        ...
        with self._conn() as conn:
            cur = conn.execute(
                """
                INSERT INTO retina_da_upload_log (
                    device_id, record_hash_hex, state_commitment_hex,
    ...
    def get_retina_da_upload_status(self, limit: int = 10) -> dict[str, Any]:
        limit = max(1, min(int(limit), 50))
        with self._conn() as conn:
            rows = conn.execute(
                """
                SELECT * FROM retina_da_upload_log
                ORDER BY id DESC LIMIT ?
                """,
                (limit,),
            ).fetchall()
