...
This is the canonical home of LocalLLMClient (router_orchestrator imports
it from here); HardenedNIMClient is bound at MODULE level so tests can
patch ``vapi_bridge.llm_routing.local_client.HardenedNIMClient``.
Previously this class lived in nim_cloud_client.py (now a re-export shim)
with a function-level import that was effectively unpatchable — and the
module itself was never committed, which silently degraded the
orchestrator's LOCAL_NIM provider (rescue documented in issue #143).
"""
...
except Exception:  # fail-open: heavy/optional dependency absent
    HardenedNIMClient = None  # type: ignore[assignment]
    NIMConfig = None  # type: ignore[assignment]
...
class LocalLLMClient:
    ...
    def is_available(self) -> bool:
        ...
        return bool(self._nim_client is not None and self._nim_client._config.enabled)
