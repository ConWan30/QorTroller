...
class TestBiometricFusionClassifier(unittest.TestCase):
    ...
    def test_highly_divergent_session_triggers_anomaly(self):
        ...
        import numpy as np
        clf._mean = np.array([100.0]*13, dtype=np.float64)
        clf._var  = np.array([0.01]*13, dtype=np.float64)
...
class TestFeatureVectorDimension(unittest.TestCase):
    """Phase 17/57/121: BiometricFeatureFrame now has 13 features (11 + jitter_variance + touchpad_spatial_entropy)."""

    def test_feature_vector_dim_11(self):
        """to_vector() returns a (13,) numpy array (11 original + Phase 57 jitter_variance + Phase 121 touchpad_spatial_entropy)."""
        frame = BiometricFeatureFrame()
        vec = frame.to_vector()
        self.assertEqual(len(vec), 13, f"Expected 13 features, got {len(vec)}")
    ...
    def test_session_fixture_extract_dim_11(self):
        ...
        feats = BiometricFeatureExtractor().extract(snaps)
        self.assertEqual(len(feats.to_vector()), 13)
